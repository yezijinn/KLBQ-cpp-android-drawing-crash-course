...
> **本表第一部分（"完整可运行程序"）列出的代码，都是自包含、可直接编译的。**
> 正文里另有大量"讲概念用的短片段"，它们**会省略 `#include` 或依赖前文定义**，
> 不能单独编译——这不是错误，是片段展示的默认规则（详见 [[第02章-装好第一套工具链]] 的"贯穿全书的约定"）。
> 要动手跑，**优先用本表列出的完整程序，或各章"动手"一节的代码**。
...
| 24 | 骨骼链手算 | 48 | ~60 | 父级×局部=世界 | `g++ -std=c++17` |
