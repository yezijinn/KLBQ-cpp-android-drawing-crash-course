...
- [ ] **理解后端六步**：缓冲管理→上传→渲染通道→投影→遍历命令→结束
- [ ] **看投影参数**：找到 `constants[4]`（scale + translate）→ 对应顶点着色器的 push constant
- [ ] **CPU 后端实验**：用第 62 章的 CPU 渲染后端把 ImGui 画到图片 → 验证三角形生成正确
- [ ] **数顶点**：一个矩形 = 4 顶点 + 6 索引（2 个三角形）
...
### 习题 62.1 CPU 后端验证三角形数（★★，应用变式）

**任务要求**：用本章的 CPU 光栅化后端，画以下内容并打印顶点数/索引数：

1. 一个矩形（`AddRectFilled`）
2. 一条线（`AddLine`）
3. 一段文字"AB"
...
```cpp
// 矩形 = 2 三角形 = 4 顶点 / 6 索引
// 线   = 1 四边形（或退化）= 4 顶点 / 6 索引
// 文字 "AB" = 2 字符 × 2 三角形 = 4 三角形
printf("矩形: vtx=%d idx=%d\n", 4, 6);
```

**验证断言**：矩形 4 顶点 6 索引；文字每个字符 4 顶点 6 索引。

### 习题 62.2 分析"缓冲区为什么要扩容"（★★★，分析+评价）

**任务要求**：本章后端"若缓冲不够大就重新分配"。分析：

1. 为什么**不**每帧重新分配 GPU 缓冲？
2. 为什么扩容是"1.5 倍"而不是"刚好够"？
3. 如果顶点数**突然暴增**（比如一帧画几千个 ESP 框），会怎样？

**参考答案要点**：
1. GPU 内存分配很慢（要驱动参与），每帧分配合拖垮帧率；
2. 按比例扩容减少**重分配次数**（均摊 O(1)），若刚好够则下一帧多一个顶点就要再分；
3. 会触发一次重分配（卡一帧），之后恢复；极端情况可**预分配上限**避免抖动。

> [!tip] 评价层要点
> 这是"**空间换时间**"的经典权衡：多占一点内存（1.5 倍），换掉每帧分配的延迟。
...
- [ ] 知道为什么要扩容缓冲而不是每次重建（说出"重建开销大"）
- [ ] 知道 `loadOp = CLEAR` 配合透明清屏色 = 透明背景（说出两者配合）
- [ ] 能读懂正交投影矩阵的每个元素（逐元素说出含义）
- [ ] 知道 scissor 用于硬件裁剪（说出"只画指定矩形区域"）
- [ ] 知道 `vtx_offset` 为什么要累积（说出"多个 DrawCmd 共用一个大顶点缓冲"）
- [ ] 写了 CPU 后端并输出了图片（运行后端，生成 PPM 图片并能打开）
