...
### 习题 61.1 数一数 DrawList 的批次（★★，应用变式）

**任务要求**：写一段代码，用 ImGui 画 3 个不同纹理的元素（例如字体文本 + 一个自定义图标 + 另一段文本），
打印出 `CmdBuffer.Size`，并解释为什么是这个数。

**参考骨架**：

```cpp
ImDrawList* draw = ImGui::GetForegroundDrawList();
draw->AddText(font, 16, {10,10}, IM_COL32_WHITE, "hello");
draw->AddImage(myIcon, {100,10}, {132,42});        // 换纹理
draw->AddText(font, 16, {200,10}, IM_COL32_WHITE, "world");

printf("命令数 = %d\n", draw->CmdBuffer.Size);
// 预期：3（按纹理/裁剪分组，切一次纹理一条命令）
```

**验证断言**：命令数 = "纹理切换次数 + 1"，能说出"为什么要按纹理分组"。

### 习题 61.2 分析"立即模式为什么适合覆盖层"（★★★，分析+评价）

**任务要求**：对比"保留模式"和"立即模式"，分析：

1. 覆盖层的 UI 有什么特点（复杂吗？变吗？）？
2. 立即模式在"数据频繁变化"时有什么优势？
3. 立即模式在什么场景下**不**合适？

**参考答案要点**：
1. 覆盖层 UI 简单（开关、滑块、少量文本），但**数据每帧变**（坐标、血量、距离）；
2. 每帧重建，UI 显示值和变量值**天然一致**，不会"UI 与状态不同步"；
3. 复杂表单、大量控件、需要精细状态管理的应用 UI 不适合（每帧重建开销大、状态管理麻烦）。

> [!tip] 评价层要点
> 选型的核心是**"UI 复杂度 vs 数据变化频率"**。
> 覆盖层是"简单 UI + 高频数据"，恰好是立即模式的最佳场景。
