...
### 习题 58.1 画出 Vulkan 对象层级（★★，应用变式）

**任务要求**：不看本章图，凭记忆画出五个核心对象的层级关系，
并标注"谁创建谁"（用箭头）。
...
```
VkInstance
   ├──枚举──▶ VkPhysicalDevice（vkEnumeratePhysicalDevices，不是创建）
   │              └──创建──▶ VkDevice
   │                            ├──▶ VkQueue（从设备取）
   │                            ├──▶ VkCommandPool
   │                            └──▶ VkPipeline
...
**验证断言**：能说出"物理设备是枚举出来的，逻辑设备才是创建的"。

### 习题 58.2 分析"为什么物理设备和逻辑设备要分开"（★★★，分析）

**任务要求**：Vulkan 有 `VkPhysicalDevice`（物理）和 `VkDevice`（逻辑）两层。
分析：

1. 为什么不能只用一层？
2. 一个进程能有几个 `VkInstance`？几个 `VkPhysicalDevice`？几个 `VkDevice`？
3. 这对"多 GPU 选择"有什么好处？

**参考答案要点**：
1. 物理设备是"硬件事实"（只读、用来查询能力），逻辑设备是"你的连接"（可配置、可销毁）；
2. 通常 1 个 Instance；物理设备可能有多个（核显+独显）；逻辑设备可以创建多个（各自独立配置）；
3. 可以先枚举所有物理设备、查属性、选最合适的一个再创建逻辑设备——**选择与使用分离**。

> [!tip] 评价层要点
> 这是典型的"**查询/配置分离**"设计：物理设备层负责"看"（能力、内存、队列族），
> 逻辑设备层负责"用"。分开后，程序能在**创建前**做决策，而不是创建后才发现不合适。
