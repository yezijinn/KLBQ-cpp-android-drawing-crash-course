...
> [!danger] 【安全最佳实践】root 是"能力"不是"默认"
> **威胁机制**：
> ① 以 root 运行的程序一旦被利用，攻击者获得**全设备控制权**；
> ② `setenforce 0`（关 SELinux）会让**整个系统**失去强制访问控制；
> ③ 提权后误操作（如 `rm -rf /`）不可恢复。
> **规避原则**：
> - **最小权限**：只在必需时提权，用完即降权；
> - **不永久关 SELinux**（`setenforce 0` 只用于临时诊断，第 37 章强调过）；
> - 提权操作加**二次确认**，危险命令（rm/格式化）先 dry-run；
> - 生产环境用 **capability**（细粒度特权）替代全 root。
>
> 本教程的跨进程读取等练习，**在你自己授权的设备上做**——这是底线。
