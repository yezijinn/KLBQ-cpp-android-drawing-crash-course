...
### 习题 44.1 复刻 KLBQ 的目录骨架（★★，应用变式）

**任务要求**：按本章三条约定，为你的工程建出**同构目录**（`include/` 与 `src/` 镜像），
并让 `Android.mk` 的 `LOCAL_C_INCLUDES` 指向它。
...
│   ├── Application.mk
│   ├── include/
│   │   └── My_Utils/  VectorTools.h
│   └── src/
│       ├── main.cpp
│       └── My_Utils/  GameTools.cpp
```

**验证断言**：`#include "VectorTools.h"`（从 include 根算）能编译通过，不需要写相对路径。

### 习题 44.2 分析"目录设计"（★★★，分析+评价）

**任务要求**：本章给了"按功能分层"（`include/Vulkan/`）和"按类型分层"（`include/headers/`）两种方案。
分析：

1. 为什么本项目选"按功能分层"？
2. "按类型分层"在什么规模下会失控？
3. 本章提到 `driver.h` 放在 `src/` 是"历史包袱"——为什么说它是包袱？

**参考答案要点**：
1. 改一个功能只碰一个目录，**内聚性高**；镜像结构让 .h ↔ .cpp 好找；
2. 项目一大，`headers/` 会堆成几十个不相关的头，**没有任何边界**；
3. `driver.h` 是**公共接口**，理应在 `include/` 供外部引用；现在靠额外 `LOCAL_C_INCLUDES` 硬加路径，**破坏了"include/ 是公共接口"的一致性**，新人容易找不到。

> [!tip] 评价层要点
> 判断目录结构好坏的标准只有一个：**"改一处功能，要动几个目录？"**
> 动得越少越好。这就是"高内聚"在目录层面的体现。
