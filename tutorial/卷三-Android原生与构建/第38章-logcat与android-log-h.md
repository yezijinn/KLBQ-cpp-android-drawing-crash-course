...
> [!danger] 【安全最佳实践】Dump 目录别用 0777 + /sdcard
> **威胁机制**：
> ① `mkdir(..., 0777)` 给了**所有人读写执行**权限，同设备任何 App 都能篡改/删除 dump 文件；
> ② `/sdcard` 是**全局可读**的共享存储，dump 出的内存数据（可能含敏感地址、模块布局）会被其它 App 读取；
> ③ 结合第 38 章日志脱敏的教训——dump 文件本身就是"落盘的日志"，同样可能泄露。
>
> **规避原则**：
> - 目录权限用 `0700`（仅主人）或 `0770`（主人+同组），**绝不用 `0777`**；
> - 放**私有目录**（如 App 私有空间、`/data/local/tmp`），不放 `/sdcard`；
> - dump 完成后如非必要，**及时删除**或加密。
>
> 这是本项目 `DumpMemory` 的已知可优化点——教学时保留原始写法，但**生产环境应收紧权限**。
...
> [!danger] 【安全最佳实践】日志脱敏——最常见的敏感信息泄露点
> `logcat` 是**公开可读**的（同设备任何 App + `adb` 都能看）。
> **威胁机制**：日志里打印了密钥/密码/用户数据/内存地址 → 任何能读 logcat 的人都能拿到。
> **规避原则**：
> - **密钥/密码绝不进日志**（要打印就掩码：`KEY=****`）；
> - 用户敏感数据（账号、位置）脱敏后再打；
> - 生产版本用 `#if DEBUG` 或宏开关**关掉**调试日志（第 10 章的开关式裁剪）；
> - 日志级别控制：敏感信息用 DEBUG 级，release 不输出。
