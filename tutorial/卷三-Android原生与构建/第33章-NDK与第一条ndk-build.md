...
> [!tip] 选型权衡：为什么用 NDK？（供应链卡片）
> **本项目锁定版本**：**NDK r27c**（教程全程以 `C:\dev\android-ndk-r27c` 为例）。
>
> | 维度 | 说明 |
> |---|---|
> | **为何选它** | Android 上写 C/C++ 的**唯一官方工具链**（含交叉编译器 + 系统头文件 + 库） |
> | **核心优势** | 自带 clang、`ndk-build`、LLVM 工具（readelf/nm/addr2line）、arm64 sysroot |
> | **在项目中的角色** | 编译整个 KLBQ（`ndk-build` 驱动），卷五~卷八都靠它 |
> | **供应链风险** | 版本间行为有差异（如 `APP_PLATFORM` 支持范围、默认 STL）；**路径不能含中文/空格** |
>
> **版本兼容提示**：教程命令里的 **`r27c` 要换成你的实际版本号**（r25c/r28 均可，
> 但本项目配置按 r27c 验证）。`APP_PLATFORM := android-25` 是编译最低版本，与实际 NDK 版本独立。
> **锁定建议**：解压到 `C:\dev\android-ndk-r27c`，目录名与版本号一致（下节说明）。
