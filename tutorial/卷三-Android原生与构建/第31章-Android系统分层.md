...
### 习题 31.1 给 KLBQ 的每个调用"贴层标签"（★★，分析变式）

> [!note] 这道题练什么
> 本章给你一张"五层结构图"。这道题是**应用变式**：把图用起来——
> 拿本项目真实调用的几个 API，判断它属于哪一层、经由什么机制。

**任务要求**：下面 6 个 KLBQ 实际用到的能力，各属于 Android 的哪一层？
填表并说明"这一层是靠什么机制提供服务"。

| # | 能力（本项目真实调用） | 属于哪一层 | 服务机制 |
|---|---|---|---|
| 1 | `__android_log_print` 写日志 | ？ | ？ |
| 2 | `ANativeWindow_fromSurface` 拿画布 | ？ | ？ |
| 3 | `dlopen("libgui.so")` 加载图形私有库 | ？ | ？ |
| 4 | `process_vm_readv` 读别的进程内存 | ？ | ？ |
| 5 | `SurfaceComposerClient::createSurface` 申请图层 | ？ | ？ |
| 6 | `__system_property_get("ro.build.version.sdk")` | ？ | ？ |

**参考答案要点**：

| # | 层 | 机制 |
|---|---|---|
| 1 | 原生库层 | `liblog.so`（NDK 公开） |
| 2 | 原生库层 | `libandroid.so`（NDK 公开） |
| 3 | 原生库层 | 私有 `.so` + `dlopen`（绕过 namespace） |
| 4 | Linux 内核 | 系统调用（第 74 章） |
| 5 | 原生库层 | 私有 `libgui`，内部走 Binder 到 surfaceflinger |
| 6 | 原生库层 | `libc` 的 `__system_property_get` 读 property 区 |

> [!tip] 关键区分
> 第 1、2、3、5、6 都落在"原生库层"，但**入口机制不同**：
> NDK 公开 API vs 私有库 dlopen vs 系统调用。
> 本项目之所以能站在原生库层，正是因为它是**独立可执行文件**（第 32 章）。

### 习题 31.2 诊断"悬浮窗不显示"（★★★，分析+评价）

**场景**：你写的 KLBQ 编译成功、push 成功、运行也不崩，但**屏幕上什么都没出现**。
按本章的分层图，从下往上列出**至少 4 个可能的失败环节**，并给出每一环的**验证命令**。

**提示骨架**（自己补完）：

| 环节 | 可能原因 | 验证命令 |
|---|---|---|
| 内核/权限 | 没 root，申请图层被拒 | `adb shell id` → 看是否 uid=0 |
| 原生库层 | `dlopen("libgui.so")` 失败 | ？ |
| 合成层 | 申请了图层但没画内容 | ？ |
| 图层层级 | z-order 太低被盖住 | ？ |
| 程序逻辑 | 主循环立即退出 | ？ |

**评价要求**：指出哪一环**最可能**，并说明理由（结合"本项目最容易出问题的是权限和 dlopen"）。

> [!note] 与后续章节的呼应
> 这道题的完整答案散落在第 37、64~67 章。
> 现在做不出来正常——**重点是建立"分层排查"的思维**，而不是背答案。
