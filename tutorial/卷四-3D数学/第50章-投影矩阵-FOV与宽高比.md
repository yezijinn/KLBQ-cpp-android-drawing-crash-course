...
**定义 `t = tan(FOV/2)`**：它把"深度"换算成"**归一化后的半屏尺寸**"（NDC = Normalized Device Coordinates，归一化设备坐标，范围 [-1,1]，里面屏幕边缘是 ±1）。
到这里 t 是从垂直 FOV 推出来的。
...
| 症状 | 最可能的原因 | 解法 |
|---|---|---|
| 画面**上下颠倒** | 忘了 Y 轴翻转（`1 - ndc_y`） | 确认是 `(1 - ndc_y)` 不是 `(ndc_y + 1)` |
| 画面**左右镜像** | 横向符号反了 | 检查 `ndc_x` 前的符号 |
| 物体**贴在屏幕中心不散开** | `t` 算成了 1（FOV 传了 90 但没转弧度） | `t = tanf(FOV * 0.5f * PI / 180.0f)`，别忘了 `/ 180 * PI` |
| 远近物体**一样大** | 漏了透视除法（没除以 `w`） | `ndc = clip / w`，`w` 是深度 |
| 屏幕坐标**超出画布** | `px/py` 用了整宽整高，不是半宽半高 | `px = 宽/2`、`py = 高/2` |
