...
## 综合实战：给 mathlib 补一套完整测试（脱离指导）

> [!important] 独立完成
> 给第 53 章的 `mathlib` 写一套测试，覆盖**边界 + 常规 + 回归**三类。

**业务需求**：mathlib 要交付给全项目用，任何改动都必须先跑测试。写 `mathlib_test.cpp`：
1. **常规用例**：向量加减/点积/叉积/长度（对照手算值）
2. **边界用例**：零向量归一化、`acos` 输入 clamp、角度归一化跨 ±180
3. **回归用例**：曾经改错过的地方（如矩阵第 15 项符号）
...
int main() {
    using namespace ml;
    // TODO: 常规用例 —— 至少 6 条
    // TODO: 边界用例 —— 至少 4 条（零向量/acos/角度归一化/除零）
    // TODO: 回归用例 —— 至少 2 条
    printf("通过 %d, 失败 %d\n", passed, failed);
    return failed == 0 ? 0 : 1;   // 失败返回非 0，便于 CI
}
```
...
|---|---|
| 失败时返回非 0（CI 用）| 工程化 |
| 每条测试有**名字**（失败时能定位）| 可维护性 |
| 边界用例覆盖零向量/NaN/clamp | 边界思维 |
| 用 `Near()` 而非 `==` 比较浮点 | 浮点理解 |
| 故意改坏 mathlib 一处，测试**变红** | 测试有效性验证 |

**验收**：`./mathlib_test` 通过 12+ 条；故意改坏一处后**至少一条变红**。

## 课后习题

### 习题 54.1 补 6 条边界测试（★★，应用变式）

**任务要求**：本章测试偏"常规值"。请补 6 条**边界测试**，覆盖：
...
CHECK_NEAR(z.LengthSq(), 0.0f);           // 不 NaN

TEST("边界：acos clamp");
float c = Vec3(1,0,0).Normalized().Dot(Vec3(1,1e-7f,0).Normalized());
c = std::clamp(c, -1.0f, 1.0f);
CHECK(std::isfinite(std::acos(c)));

TEST("边界：±180 归一化");
CHECK_NEAR(NormalizeAngle(180), 180);
CHECK_NEAR(NormalizeAngle(-180), 180);    // 注意边界归属

TEST("边界：FOV=1 与 179");
Camera c1; c1.Position={0,0,50}; c1.FOV=1;   CHECK(c1.BuildMatrix());
Camera c2; c2.Position={0,0,50}; c2.FOV=179; CHECK(c2.BuildMatrix());
```

**验证断言**：6 条全绿；把 `Normalized` 的防除零删掉，第 1 条应**变红**。

### 习题 54.2 分析"变异测试为什么比覆盖率更有效"（★★★，分析+评价）

**任务要求**：本章介绍了变异测试（故意植入错误，看测试能否抓到）。
写一段 120~200 字，回答：

1. "测试覆盖率高"为什么**不代表**测试有效？
2. 变异测试**抓的是什么**？
3. 给出一个"覆盖率高但没抓到错误"的具体例子。

**参考答案要点**：
1. 覆盖率只统计"代码被执行过"，**不检查断言是否正确**——`assert(true)` 也能"覆盖"；
2. 变异测试抓的是"**测试能否发现行为改变**"，即断言是否真的在验证逻辑；
3. 例：W2S 里 `1.0f - ndc` 改成 `1.0f + ndc`，代码**照样执行**（覆盖 100%），但若测试只检查"函数返回 true"而不检查具体坐标值，就抓不到。

> [!tip] 评价层要点
> **覆盖率衡量"有没有跑到"，变异测试衡量"跑到了能不能发现错"。**
> 前者是必要条件，后者才是充分性证明。
