...
相机空间 (depth, right, up)
    ...
    │  ② 除以 depth、除以 t      →  归一化坐标
    ↓
NDC (-1..1)   ← 归一化设备坐标（Normalized Device Coordinates）   ← 归一化设备坐标（Normalized Device Coordinates）   ← 归一化设备坐标（Normalized Device Coordinates）   ← 归一化设备坐标（Normalized Device Coordinates）
    │
    │  ③ 映射到像素、翻转 Y
    ↓
屏幕坐标 (sx, sy)
