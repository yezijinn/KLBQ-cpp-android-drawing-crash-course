...
### 习题 90.1 一次读完整个 CameraCache（★★，应用变式）

**任务要求**：用本章的 `CameraCache` 结构，把三次独立 `Read` 合并成**一次**读取。
...
CameraCache cc;
dr->Read((uintptr_t)玩家相机 + 0x2300, &cc, sizeof(cc));
// 现在 cc.location / cc.rotation / cc.fov 都有值
```

**验证断言**：一次读取的结果与三次独立读取一致；且**减少 2 次系统调用**。

> [!note] 读外部内存时，本地结构体布局必须与目标进程一致
> 本例 `CameraCache` 三个字段（Vec3 12 字节 + Rotator 12 字节 + float 4 字节 = 28 字节）
> **恰好自然对齐、无填充**，所以加不加 `#pragma pack(1)` 结果都一样（已实测 sizeof=28、偏移 0/12/24）。
>
> 但**通用原则**是：读外部内存时，本地定义要与目标进程的内存布局**逐字节一致**。
> 一旦结构体里出现 `char` 后接 `int`、或 `double` 等情况，默认对齐就会插入填充字节，
> 此时 `offsetof` 与目标布局不符，读出来就是错的。**所以工程上习惯性加 `#pragma pack(1)` 作为防御**，
> 并用 `static_assert(sizeof(T) == 期望字节数)` 校验。

### 习题 90.2 分析"自瞄该写 ControlRotation 还是 POV"（★★★，分析+评价）

**任务要求**：本章区分了"意图"（ControlRotation）和"结果"（POV）。分析：

1. 自瞄要"瞄准敌人"，应该修改哪个？为什么？
2. 如果写了 POV 会有什么副作用？
3. 什么情况下两者会不同步？

**参考答案要点**：
1. 写 **ControlRotation**——因为它是"玩家输入"，引擎会据此**自然地**驱动相机转向，看起来像"人在转视角"；
2. 直接写 POV 是"硬塞"相机位置，可能**被引擎每帧覆盖**、或导致视角与输入不一致（抖动/回弹）；
3. 过场动画、开镜插值、载具/特殊状态。

> [!tip] 评价层要点
> 核心是"**改输入 vs 改输出**"：改输入让系统自然演化（稳定），改输出是绕过系统（易冲突）。
> 自瞄选前者，是为了"看起来自然且不被引擎纠正"。
