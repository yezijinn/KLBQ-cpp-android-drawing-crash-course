...
> [!important] 前置条件
> - 本章让你**自己造 demo 引擎**（不读真实游戏），可在**任意平台本机编译运行**。
> - 这是卷七推荐的练习靶子：你造的，你知道每个字段真值。
...
| 症状 | 最可能的原因 | 解法 |
|---|---|---|
| 遍历对象**漏掉最后一个** | 循环用了 `i <= Count` 或 `i < Count-1` | 数组下标 `0..Count-1`，用 `i < Count` |
| 读组件**崩了** | 组件链表某环是空指针 | 每跳判空（第 89 章）|
