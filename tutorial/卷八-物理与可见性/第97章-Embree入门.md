...
> [!note] 承上
> 上一章手写了 BVH，理解了原理。
> 本章用**现成的高性能库 Embree**（第 43 章已接进工程）——四个核心对象 + 标准流程。
...
- [ ] 会走完九步流程 ★（设备→场景→几何→顶点→索引→commit→attach→commit→查询）
- [ ] 知道 `geomID` 的用途（取回对应网格）（说出"命中后据此找是哪个几何"）
- [ ] **知道每次查询前必须重置 `hit.geomID`** ★（写出 `hit.geomID = RTC_INVALID_GEOMETRY_ID`）
- [ ] 知道 `rtcOccluded1` 比 `rtcIntersect1` 快（说出"只判断有没有挡，不求最近点"）
- [ ] 知道 `threads` 参数在手机上要限制（说出"否则线程爆炸"）
- [ ] 跑通了完整示例，四个查询结果正确（4 个用例命中/未命中正确）
- [ ] 实测了性能，与手写 BVH 对比过（输出 Embree 耗时，与手写对比）
