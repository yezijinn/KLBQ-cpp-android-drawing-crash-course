...
|---|---|---|
| 暴力遍历 | O(T) | — |
| 平衡 BVH | O(log T) | O(T log T) |
| 手写简易 BVH | 平均 O(log T) | O(T log T) |

T = 三角形数。10000 三角形：暴力 10000 次 vs BVH ~log₂(10000)≈14 层。

**【空间开销】**
- 每个 BVH 节点存一个 AABB（6 float = 24 字节）+ 子节点指针。
- 总节点数 ≈ 2T（二叉树），空间 O(T)。

**【并发与 I/O 瓶颈】**
- 构建是纯 CPU（可并行）；查询是只读的（多线程安全）。
- 无 I/O、无锁——这是 BVH 快的根本原因。

> [!note] 工程权衡：为何手写 BVH 还要用 Embree？
> 手写 BVH 是为了**理解原理**（本章目标）；真实项目用 Embree——因为它有：
> ① 更好的 SAH 划分质量；② SIMD 加速遍历；③ 多线程构建；④ 长期维护。
> **理解原理才能正确用库**——这是本章与第 97 章的分工。
