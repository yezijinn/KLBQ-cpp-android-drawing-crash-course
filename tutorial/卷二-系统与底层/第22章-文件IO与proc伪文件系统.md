...
> [!note] 什么是"配置项字典"
> 一份配置结构体有几十个字段。把它列成表（键名/类型/默认值/作用/敏感度），
> 就是"配置项字典"——**改配置、查默认值、评估安全**都靠它。

本项目 `ConfigManager.h` 的 `Config` 结构体（节选）：

| 键名 | 类型 | 默认值 | 作用 | 敏感度 |
|---|---|---|---|---|
| `RunFPS` | int | 90 | 目标帧率 | 低 |
| `PhysX` | bool | false | 是否启用遮挡判定 | 低 |
| `syscall驱动` | bool | false | 用系统调用后端（否则内核驱动）| 中 |
| `方框`/`骨骼`/`射线` | bool | false | ESP 显示开关 | 低 |
| `垂直焦距系数` | float | 1.0 | W2S 微调 | 中 |
| `自瞄最大距离` | float | 50000 | 自瞄范围 | 中 |
| `触摸起手X/Y` | int | -1 | 触摸坐标 | 低 |

> [!tip] 为什么要字典化
> ① **查默认值**：忘了字段默认值？查表；
> ② **评估安全**：哪些字段敏感（如驱动模式、焦距系数）一目了然；
> ③ **文档化**：新人接手，看表就懂配置全貌。

## 多环境分层配置（Dev / Test / Prod）

> [!note] 为什么需要分层
> 开发时用假数据、测试时用模拟进程、生产时连真设备——**同一份代码，不同环境**。
> 硬编码只适合单环境；分层加载才能"一套代码多环境跑"。

**推荐范式**（本项目可选的增强）：

```cpp
// 1. 环境选择（从环境变量读，默认 dev）
const char* env = getenv("KLBQ_ENV");
std::string mode = env ? env : "dev";   // dev / test / prod

// 2. 按环境选配置源
const char* configPath =
    mode == "prod" ? "/data/Config" :          // 生产：设备上的真配置
    mode == "test" ? "/data/Config.test" :     // 测试：模拟数据
                     "/data/Config.dev";       // 开发：默认值 + 少量覆盖

// 3. 加载（缺省值兜底：文件读不全时用结构体默认值，第 22 章 LoadConfig 同款逻辑）
ConfigManager::LoadConfigFrom(configPath);
```

| 环境 | 配置文件 | 目标进程 | 特点 |
|---|---|---|---|
| **Dev** | `/data/Config.dev` | 自己写的 demo | 调试友好、日志全开 |
| **Test** | `/data/Config.test` | 模拟进程 | 可复现、无副作用 |
| **Prod** | `/data/Config` | 授权目标 | 日志精简、性能优先 |

> [!warning] 环境变量与配置文件都不该硬编码
> - 环境用**环境变量**（`getenv`）而非代码里写死；
> - 配置**不硬编码路径**（用变量）；
> - 敏感值（密钥）**不进配置文件明文**（第 24 章的安全卡片）。
...
> [!danger] 【安全最佳实践】文件权限与敏感数据落地
> **威胁机制**：
> ① 配置文件权限过宽（如 `0777`）→ 同设备其他用户可读改；
> ② 敏感数据（密钥、token）明文落盘 → 设备被物理访问即泄露；
> ③ 用 `/sdcard` 存配置 → 全局可读。
> **规避原则**：
> - 配置文件用 `0660`（主人+同组），**绝不用 `0777`**；
> - 敏感数据加密后落盘（第 24 章的教训：XOR 只防君子，真敏感用 Keystore）；
> - 放**私有目录**（如 `/data/local/tmp` 或 App 私有空间），不放 `/sdcard`；
> - 打开文件加 `O_CLOEXEC`（防子进程继承 fd）。
