...
> [!note] 两个缩写的全称
> - **PLT** = Procedure Linkage Table，**过程链接表**（放"跳板"代码）。
> - **GOT** = Global Offset Table，**全局偏移表**（放函数真实地址）。
...
> [!note] APK 是什么
> **APK = Android Package**（安卓应用安装包）：就是手机上"点图标安装 App"的那个 `.apk` 文件，
> 内部本质是个 zip，装着 Java/Kotlin 代码、资源，以及可选的 `.so` 原生库。
> 与本项目的"独立可执行文件"是两种不同形态，第 32 章详细对比。
