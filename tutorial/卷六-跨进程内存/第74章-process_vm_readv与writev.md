...
| 症状 | 最可能的原因 | 解法 |
|---|---|---|
| 返回 `-1` + `EPERM` | 权限不足（UID 不匹配且无 `CAP_SYS_PTRACE`）| 用 root 运行；查 SELinux |
| 返回 `-1` + `ESRCH` | 目标进程不存在 | 先确认进程活着，重新找 PID |
| 返回 `-1` + `EFAULT` | 地址无效（偏移失效或对象已释放）| 重新定位偏移；加强合法性过滤 |
| 返回 `> 0` 但**少于请求字节** | 目标内存中间有未映射空洞 | 正常，会读到部分；分段读或跳过空洞 |
| 读出来**全是 0** | PID/基址/偏移/字节数之一错了 | 按第 27 条清单逐项排查 |
| 编译报 `process_vm_readv 未定义` | 老 NDK 头文件没有 | 用 `syscall(270, ...)` 兜底（第 21 章）|
...
> [!danger] 【安全最佳实践】跨进程读取的授权边界
> `process_vm_readv` 能读**别的进程**的内存——这是强大的能力，也意味着责任。
> **威胁机制**：未授权读取他人进程内存，可能构成**侵犯隐私**（如读取密码、聊天记录）或
> 违反《刑法》285/286 条（非法获取计算机信息系统数据）。
> **规避原则**：
> - **只读自己写的 demo 进程**（本教程的目标进程约定）；
> - 生产工具（如性能剖析器）读目标进程需**明确授权**或**同 UID**；
> - 读取的数据**不落地**、**不外传**；
> - 调试时用 `TracerPid`（第 18 章）自查是否被反调试。
>
> 技术中立，但**用途有边界**——这是本教程反复强调的合规底线。
