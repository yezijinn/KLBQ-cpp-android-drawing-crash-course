...
### 习题 77.1 从 .dynsym 找符号（★★，应用变式）

**任务要求**：基于本章的 ELF 结构，写出"从模块基址出发，遍历 `.dynsym` 找指定符号名"的步骤骨架。

**参考骨架**：

```cpp
uint64_t FindSymbol(uint64_t base, const char* target) {
    auto* ehdr = (Elf64_Ehdr*)base;
    // 1. 用 e_phoff 遍历 Program Header，找到 PT_DYNAMIC
    // 2. 在 PT_DYNAMIC 里找 DT_SYMTAB / DT_STRTAB
    // 3. symtab 是 Elf64_Sym 数组，strtab 是字符串表
    // 4. 遍历每个 sym，比较 name = strtab + sym.st_name
    // 5. 匹配则返回 base + sym.st_value
    return 0;
}
```

**验证断言**：对已知符号能返回非 0 地址；对不存在的符号返回 0。

### 习题 77.2 分析"符号解析 vs 硬编码偏移"（★★★，分析+评价）

**任务要求**：分析：

1. 符号解析抗更新，为什么不能完全替代硬编码偏移？
2. "启动时解析一次，缓存地址"这个混合方案好在哪里？
3. 如果目标库被 strip 了，还有什么办法？

**参考答案要点**：
1. 需要**符号表存在**（很多发布版 strip 掉了），且遍历符号表**慢**；
2. 兼顾"抗更新"（每次启动重解析）和"速度快"（解析后缓存，后续当偏移用）；
3. 退化为硬编码偏移 + 多版本适配表（第 88 章），或用特征码扫描（pattern scan）。

> [!tip] 评价层要点
> 这是"**确定性 vs 鲁棒性**"的权衡：
> 符号解析鲁棒（不怕更新）但有前提条件；硬编码确定（总能用）但脆弱（一更新就废）。
> 实战选择取决于"目标是否保留符号 + 更新频率"。
