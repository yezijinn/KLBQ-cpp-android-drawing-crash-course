...
> [!IMPORTANT] 请用 Obsidian 阅读本教程（重要）
> 这套教程**不是普通 Markdown**：章节之间用 **双链 `[[...]]`** 互相跳转，用 **Callout**（`> [!note]` 彩色提示框）承载讲解，用 **任务清单** `- [ ]` 做验收打勾，还配有 **Mermaid 关系图谱** 与 **Dataview 进度看板**。
> **用 GitHub / VS Code 直接看会丢失双链跳转、折叠答案、彩色提示框和进度统计**——阅读体验大打折扣。
> **正确做法**：新建一个 Obsidian 仓库，把整个 `tutorial/` 文件夹拖进去，从 `00-开始之前.md` 开始读。详见下文「Q4」与 `附录T`。
...
- ⭐ **必须用 Obsidian 阅读**：全部用 Markdown 双链 + Callout + 任务清单 + 标签写成，**用 Obsidian 打开才能自动生成关系图谱、折叠答案、打勾验收和进度看板**（用普通编辑器阅读会丢失这些）
